...
const STATIC_WOMEN = initialCatalog.filter((p) => p.gender === "women").map(catalogToProduct);

const FILTERS = ["All", "dresses", "shirts", "pants", "shoes", "bags", "accessories", "pajamas"];
const filterLabels: Record<string, string> = {
  All: "All", dresses: "Dresses", shirts: "Tops", pants: "Pants",
  shoes: "Shoes", bags: "Bags", accessories: "Accessories", pajamas: "Pajamas",
};

export default function Women() {
  const [activeFilter, setActiveFilter] = useState("All");

  useMemo(() => { document.title = "StyleAI — Women's Collection"; }, []);

  const { data: rawProducts, isLoading, isError } = trpc.products.list.useQuery({
    gender: "women", limit: 100,
  });

  // Live data when backend is up; static catalog when it's down (GitHub Pages, no server)
  const products = useMemo(
    () => isError ? STATIC_WOMEN : (rawProducts ?? []).map(catalogToProduct),
    [rawProducts, isError],
  );
      ...
      <main className="container mx-auto px-4 py-16">
        <header className="mb-12 text-center">
          ...
          <p className="text-[#A0A0A0] uppercase tracking-[0.3em] text-[10px] font-bold">
            Curated Elegance & Modern Silhouettes
          </p>
        </header>
